...
credit_card_data = pd.read_csv('cards.csv')
#print(credit_card_data) shape (6362620, 11)
shuffle_data = credit_card_data.sample(frac=1)
#print(shuffle_data)
one_hot_data = pd.get_dummies(shuffle_data, columns=['isFraud'])
#print(one_hot_data) shape(6362620, 12)
#cc = np.asarray(one_hot_data.values, dtype='float32')
#print(cc)

normalized_data = (one_hot_data - one_hot_data.min()) / (one_hot_data.max())
#print ( normalized_data)  shape(1048575, 7)

df_X = normalized_data.drop(['isFraud_0', 'isFraud_1'], axis=1)
#print (df_X)  [1048575 rows x 5 columns]
df_y = normalized_data[['isFraud_0', 'isFraud_1']]
#print(df_y)  shape(1048575,2 )

ar_X, ar_y = np.asarray(df_X.values, dtype='float32'), np.asarray(df_y.values, dtype='float32')
#(1048575, 5)
#(1048575, 2)

train_size = int(0.8 * len(ar_X))  # allocate first 80% training 20% test
#print(train_size)838860
#######################################################
######################################################
(raw_X_train, raw_y_train) = (ar_X[:train_size], ar_y[:train_size])
(raw_X_test, raw_y_test) = (ar_X[train_size:], ar_y[train_size:])
#print (raw_X_test)  shape(209715, 5)

#print(raw_y_train)  shape (838860, 2)
#print(raw_X_train)
#print(raw_y_test)
#print(raw_y_train)

count_legit, count_fraud = np.unique( credit_card_data['isFraud'], return_counts=True)[1]
...
###############Weighting technique for imbalance class###################
# Logic weithing
weighting = 1 / fraud_ratio
print("After weigh technique :",weighting)
...
# Inputs to the model when it comes time to train it (assign values at run time)
...
def network(input_tensor):
    ...
    layer1 = tf.nn.sigmoid(
        tf.matmul(input_tensor, weight_1_node) + biases_1_node)
    # Dropout prevents model from becoming lazy and over confident
    layer2 = tf.nn.dropout(tf.nn.sigmoid(
        tf.matmul(layer1, weight_2_node) + biases_2_node), 0.85)
...
final_fraud_accuracy = calculate_accuracy(
    final_fraud_y_test, final_fraud_y_test_prediction)
